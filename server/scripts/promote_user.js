...
async function promote() {
    try {
        ...
        const result = await pool.query(
            "UPDATE users SET role = 'admin' WHERE email = $1 OR clerk_id = $1 OR name ILIKE $1 RETURNING name, email, role, clerk_id",
            [email]
        );
        ...
        if (result.rowCount === 0) {
            ...
            const allUsers = await pool.query("SELECT name, email, clerk_id, role FROM users");
            console.table(allUsers.rows);
            console.log('\nTip: Copy the "clerk_id" or "name" from the table above and run:');
